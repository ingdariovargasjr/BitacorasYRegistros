...
$borrar = $_GET['borrar'];
$sql_1 = "SELECT * FROM usuario_mla WHERE id=$borrar";
...
$backup = "SELECT * FROM usuario_mla WHERE id='$borrar'";
...
$eliminacion = "INSERT INTO bitacoras (correo_electronico, username, password, area, fecha, vinculado, accion, pers_modifico, enTabla) 
    VALUES ('" . $correo . "','" . $usuario . "', '" . $contraseña . "', '" . $area . "','" . $fecha . "','" . $vinculado . "','ELIMINACIÓN', '" . $user . "', 'MLA')";
...
$sql = "DELETE FROM usuario_mla WHERE id='$borrar'";
...
if ($result === sqlsrv_query($conn, $sql)) {
    ...
    window.location.href = 'registros_mla.php?user=<?php $user ?>';
</script>";
}
/*echo "<script>
alert('Llene todos los campos.');
