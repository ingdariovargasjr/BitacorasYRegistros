...
$backup = "SELECT * FROM usuario_multimodal WHERE id='$borrar'";
...
$eliminacion = "INSERT INTO bitacoras (correo_electronico, username, password, area, fecha, vinculado, accion, pers_modifico, enTabla) 
    VALUES ('" . $correo . "','" . $usuario . "', '" . $contraseña . "', '" . $area . "','" . $fecha . "','" . $vinculado . "','ACTIVADO', '" . $user . "', 'MULTIMODAL')";
...
//$sql = "DELETE FROM usuario_multimodal WHERE id='$borrar'";
...
/*if ($result === sqlsrv_query($conn, $sql)) {
    ...
    window.location.href = 'registros_multimodal.php?user=<?php $user ?>';
</script>";
}*/
/*echo "<script>
alert('Llene todos los campos.');
...
echo "<script>
alert('Este usuario está activado.');
